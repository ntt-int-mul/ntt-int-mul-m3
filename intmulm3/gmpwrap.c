...
void gmpmul(uint32_t cint[2*N], const uint32_t aint[N], const uint32_t bint[N]){
    ...
    // convert back to binary
    memset(cint, 0, sizeof(uint32_t)*2*N);
    mpz_export(cint, &c_size, -1, 1, -1, 0, c);
...
void gmpinvmod(uint32_t pinvint[Npad], const uint32_t pint[N]){
    mpz_t p;
    mpz_t r;
    mpz_t pinv;
    ...
    // convert back to binary
    size_t pinv_size;
    memset(pinvint, 0, sizeof(uint32_t)*Npad);
...
void gmpneginvmod(uint32_t pinvint[Npad], const uint32_t pint[N]){
    mpz_t p;
    mpz_t r;
    mpz_t pinv;
